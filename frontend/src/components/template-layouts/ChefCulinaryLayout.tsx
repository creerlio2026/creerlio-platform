...
export default function ChefCulinaryLayout(props: LayoutProps) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-50 via-amber-50 to-white text-slate-900">
      ...
      <div className="h-[50vh] relative">
        {bannerUrl && templateState.include_banner ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-orange-400 via-amber-400 to-orange-600" />
        )}
      ...
      <div className="max-w-6xl mx-auto px-8 py-12">
        ...
        {introVideoUrl && templateState.include_intro_video && (
          <section className="mb-10 relative bg-white rounded-lg shadow-lg p-6 border-t-4 border-orange-500">
            {editMode && <div className="mb-4 flex justify-end"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={templateState.include_intro_video} onChange={onToggleIntroVideo} className="w-5 h-5 rounded border-2 border-orange-500" /><span className="text-sm text-slate-600">Include Intro Video</span></label></div>}
            <h2 className="text-2xl font-bold mb-4 text-orange-700">Introduction Video</h2>
            <div className="max-w-4xl mx-auto"><video src={introVideoUrl} controls className="w-full rounded-lg border-2 border-orange-200" style={{ maxHeight: '600px' }}>Your browser does not support the video tag.</video></div>
          </section>
        )}
        {templateState.included_sections.includes('skills') && visibleSkills.length > 0 && (
          <section className="mb-10 relative bg-white rounded-lg shadow-lg p-6 border-t-4 border-orange-500">
            ...
            <h2 className="text-2xl font-bold mb-4 text-orange-700">Culinary Skills</h2>
            <div className="flex flex-wrap gap-2">{(portfolioData.skills || []).map((skill, idx) => {
              ...
              return <div key={idx} className={`relative px-4 py-2 rounded-full text-sm font-semibold ${editMode && !isSelected ? 'bg-slate-100 text-slate-400 line-through' : 'bg-orange-100 text-orange-700 border-2 border-orange-300'}`}>{skill}{editMode && <label className="absolute -top-1 -right-1 cursor-pointer"><input type="checkbox" checked={isSelected} onChange={() => onToggleItem('skills', idx)} className="w-4 h-4 rounded border-2 border-orange-500" /></label>}</div>
            })}</div>
          </section>
        )}
        {templateState.included_sections.includes('experience') && visibleExperience.length > 0 && (
          <section className="mb-10 relative bg-white rounded-lg shadow-lg p-6 border-t-4 border-orange-500">
            ...
            <h2 className="text-2xl font-bold mb-4 text-orange-700">Culinary Experience</h2>
            <div className="space-y-4">{(portfolioData.experience || []).map((exp, idx) => {
              ...
              if (!editMode && !isSelected) return null
              return <div key={idx} className={`relative border-l-4 border-orange-400 pl-6 py-4 ${editMode && !isSelected ? 'opacity-40' : 'bg-orange-50 rounded-lg'}`}>{editMode && <label className="absolute top-2 right-2 cursor-pointer"><input type="checkbox" checked={isSelected} onChange={() => onToggleItem('experience', idx)} className="w-5 h-5 rounded border-2 border-orange-500" /></label>}<div className="font-bold text-lg text-orange-700">{exp.title || exp.role || 'Role'}</div><div className="text-orange-600 mb-2">{exp.company || 'Company'}</div>{exp.description && <p className="text-slate-700 whitespace-pre-wrap">{exp.description}</p>}</div>
            })}</div>
          </section>
        )}
        {templateState.included_sections.includes('projects') && visibleProjects.length > 0 && (
          <section className="mb-10 relative bg-white rounded-lg shadow-lg p-6 border-t-4 border-orange-500">
            ...
            <h2 className="text-2xl font-bold mb-4 text-orange-700">Signature Dishes & Menus</h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">{(portfolioData.projects || []).map((project, idx) => {
              ...
              if (!editMode && !isSelected) return null
              return <div key={idx} className={`relative border-2 border-orange-200 rounded-lg p-4 aspect-square bg-gradient-to-br from-orange-50 to-amber-50 ${editMode && !isSelected ? 'opacity-40' : ''}`}>{editMode && <label className="absolute top-2 right-2 cursor-pointer"><input type="checkbox" checked={isSelected} onChange={() => onToggleItem('projects', idx)} className="w-5 h-5 rounded border-2 border-orange-500" /></label>}<h3 className="font-bold text-orange-700 mb-2">{project.name || project.title || 'Dish'}</h3>{project.description && <p className="text-slate-700 text-sm whitespace-pre-wrap">{project.description}</p>}</div>
            })}</div>
          </section>
        )}
        {templateState.included_sections.includes('education') && visibleEducation.length > 0 && (
          <section className="mb-10 relative bg-white rounded-lg shadow-lg p-6 border-t-4 border-orange-500">
            ...
            <h2 className="text-2xl font-bold mb-4 text-orange-700">Education & Training</h2>
            <div className="space-y-4">{(portfolioData.education || []).map((edu, idx) => {
              ...
              return <div key={idx} className={`relative border-l-4 border-orange-400 pl-6 py-4 ${editMode && !isSelected ? 'opacity-40' : 'bg-orange-50 rounded-lg'}`}>{editMode && <label className="absolute top-2 right-2 cursor-pointer"><input type="checkbox" checked={isSelected} onChange={() => onToggleItem('education', idx)} className="w-5 h-5 rounded border-2 border-orange-500" /></label>}<div className="font-bold text-lg text-orange-700">{edu?.degree || edu?.qualification || 'Degree'}</div><div className="text-orange-600 mb-2">{edu?.institution || edu?.school || 'Institution'}{edu?.year && ` • ${edu.year}`}</div>{edu?.notes && <p className="text-slate-700 whitespace-pre-wrap">{edu.notes}</p>}{attachmentIds.length > 0 && <div className="mt-4"><div className="text-xs text-slate-500 mb-2">Attached documents: <span className="text-slate-700 font-semibold">{attachmentIds.length}</span></div><div className="space-y-2">{attachmentIds.slice(0, 3).map((id: any) => {const numId = typeof id === 'number' ? id : Number(id);if (!Number.isFinite(numId) || numId <= 0) return null;const it = tbItemCache[numId];if (!it) return <div key={numId} className="text-xs text-slate-400 p-2 bg-slate-50 rounded">Loading document {numId}…</div>;return <div key={numId} className="flex items-center gap-3 p-2 bg-slate-50 rounded border border-orange-200"><div className="text-sm text-slate-700">{it?.title || 'Document'}</div><div className="text-xs text-slate-500">{it?.item_type || ''}</div></div>})}{attachmentIds.length > 3 && <div className="text-xs text-slate-400">+{attachmentIds.length - 3} more…</div>}</div></div>}
            </div>
          })}
            </div>
          </section>
        ...
        {templateState.included_sections.includes('referees') && visibleReferees.length > 0 && (
          <section className="mb-10 relative bg-white rounded-lg shadow-lg p-6 border-t-4 border-orange-500">
            ...
            <h2 className="text-2xl font-bold mb-4 text-orange-700">Referees</h2>
            <div className="space-y-4">{(portfolioData.referees || []).map((ref, idx) => {
              ...
              return <div key={idx} className={`relative border-l-4 border-orange-400 pl-6 py-4 ${editMode && !isSelected ? 'opacity-40' : 'bg-orange-50 rounded-lg'}`}>{editMode && <label className="absolute top-2 right-2 cursor-pointer"><input type="checkbox" checked={isSelected} onChange={() => onToggleItem('referees', idx)} className="w-5 h-5 rounded border-2 border-orange-500" /></label>}<div className="font-bold text-lg text-orange-700">{ref?.name || 'Referee'}</div><div className="text-orange-600 mb-2">{ref?.title && `${ref.title} • `}{ref?.company || ''}</div>{(ref?.email || ref?.phone) && <div className="text-slate-500 text-sm mb-2">{ref?.email && <div>Email: {ref.email}</div>}{ref?.phone && <div>Phone: {ref.phone}</div>}</div>}{ref?.notes && <p className="text-slate-700 whitespace-pre-wrap">{ref.notes}</p>}{attachmentIds.length > 0 && <div className="mt-4"><div className="text-xs text-slate-500 mb-2">Attached documents: <span className="text-slate-700 font-semibold">{attachmentIds.length}</span></div><div className="space-y-2">{attachmentIds.slice(0, 3).map((id: any) => {const numId = typeof id === 'number' ? id : Number(id);if (!Number.isFinite(numId) || numId <= 0) return null;const it = tbItemCache[numId];if (!it) return <div key={numId} className="text-xs text-slate-400 p-2 bg-slate-50 rounded">Loading document {numId}…</div>;return <div key={numId} className="flex items-center gap-3 p-2 bg-slate-50 rounded border border-orange-200"><div className="text-sm text-slate-700">{it?.title || 'Document'}</div><div className="text-xs text-slate-500">{it?.item_type || ''}</div></div>})}{attachmentIds.length > 3 && <div className="text-xs text-slate-400">+{attachmentIds.length - 3} more…</div>}</div></div>}
            </div>
          })}
            </div>
          </section>
        ...
        {templateState.included_sections.includes('attachments') && visibleAttachments.length > 0 && (
          <section className="mb-10 relative bg-white rounded-lg shadow-lg p-6 border-t-4 border-orange-500">
            ...
            <h2 className="text-2xl font-bold mb-4 text-orange-700">Attachments</h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">{(portfolioData.attachments || []).map((att, idx) => {
              ...
              return <div key={idx} className={`relative border-2 border-orange-200 rounded-lg p-4 aspect-square bg-gradient-to-br from-orange-50 to-amber-50 ${editMode && !isSelected ? 'opacity-40' : ''}`}>{editMode && <label className="absolute top-2 right-2 cursor-pointer"><input type="checkbox" checked={isSelected} onChange={() => onToggleItem('attachments', idx)} className="w-5 h-5 rounded border-2 border-orange-500" /></label>}{url && <a href={url} target="_blank" rel="noopener noreferrer" className="block"><h3 className="font-bold text-orange-700 mb-2">{att?.title || 'Attachment'}</h3><p className="text-sm text-slate-500">{att?.file_type || att?.item_type || ''}</p></a>}
            </div>
          })}
            </div>
          </section>
        ...
        {templateState.included_sections.includes('social') && (portfolioData.socialLinks || []).length > 0 && (
          <section className="relative bg-white rounded-lg shadow-lg p-6 border-t-4 border-orange-500">
            {editMode && <div className="mb-4 flex justify-end"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={true} onChange={() => onToggleSection('social')} className="w-5 h-5 rounded border-2 border-orange-500" /><span className="text-sm text-slate-600">Include Section</span></label></div>}
            <h2 className="text-2xl font-bold mb-4 text-orange-700">Connect</h2>
