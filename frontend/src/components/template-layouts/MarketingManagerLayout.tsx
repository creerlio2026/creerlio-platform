...
export default function MarketingManagerLayout(props: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center bg-gradient-to-br from-blue-600 via-cyan-600 to-blue-800 text-white overflow-hidden">
        {bannerUrl && templateState.include_banner && (
          <div className="absolute inset-0 z-0">
            <img src={bannerUrl} alt="Banner" className="w-full h-full object-cover opacity-30" />
            <div className="absolute inset-0 bg-gradient-to-br from-blue-600/90 via-cyan-600/90 to-blue-800/90"></div>
          </div>
        )}
        <div className="relative z-10 max-w-7xl mx-auto px-8 py-20 text-center w-full">
          ...
          {templateState.included_sections.includes('intro') && (
            <div className="flex flex-col items-center">
              ...
              <p className="text-2xl md:text-3xl text-blue-100 mb-6 font-light">{portfolioData.title}</p>
              ...
              {portfolioData.yearsExperience && (
                <div className="mt-8 text-xl text-blue-100 font-semibold">
      ...
      <div className="max-w-7xl mx-auto px-8">
        ...
        {templateState.included_sections.includes('skills') && visibleSkills.length > 0 && (
          <section className="py-16 relative bg-gradient-to-br from-blue-50 to-cyan-50">
            {editMode && <div className="mb-4"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={true} onChange={() => onToggleSection('skills')} className="w-5 h-5 rounded border-2 border-blue-500" /><span className="text-sm text-slate-600">Include Section</span></label></div>}
            <div className="text-center mb-12">
              <h2 className="text-4xl font-bold mb-4 text-blue-700">Core Competencies</h2>
              <p className="text-slate-600 max-w-2xl mx-auto">Key skills and expertise areas</p>
            </div>
        ...
        {templateState.included_sections.includes('experience') && visibleExperience.length > 0 && (
          <section className="py-16 relative bg-white">
            {editMode && <div className="mb-4"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={true} onChange={() => onToggleSection('experience')} className="w-5 h-5 rounded border-2 border-blue-500" /><span className="text-sm text-slate-600">Include Section</span></label></div>}
            <div className="text-center mb-12">
              <h2 className="text-4xl font-bold mb-4 text-blue-700">Career Highlights</h2>
              <p className="text-slate-600 max-w-2xl mx-auto">Professional journey and achievements</p>
            </div>
            <div className="max-w-4xl mx-auto space-y-8">{(portfolioData.experience || []).map((exp, idx) => {
              ...
              if (!editMode && !isSelected) return null
              return <div key={idx} className={`relative pl-12 border-l-4 border-blue-500 ${editMode && !isSelected ? 'opacity-40' : ''}`}>
                {editMode && <label className="absolute top-2 right-2 cursor-pointer"><input type="checkbox" checked={isSelected} onChange={() => onToggleItem('experience', idx)} className="w-5 h-5 rounded border-2 border-blue-500" /></label>}
                <div className="absolute -left-3 top-0 w-6 h-6 rounded-full bg-blue-500 border-4 border-white shadow-lg"></div>
                <div className="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-lg p-6 shadow-md hover:shadow-lg transition-shadow">
                  <div className="font-bold text-xl text-blue-700 mb-1">{exp.title || exp.role || 'Role'}</div>
                  <div className="text-blue-600 mb-3 font-semibold">{exp.company || 'Company'}</div>
        ...
        {templateState.included_sections.includes('projects') && visibleProjects.length > 0 && (
          <section className="py-16 relative bg-gradient-to-br from-slate-50 to-blue-50">
            {editMode && <div className="mb-4"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={true} onChange={() => onToggleSection('projects')} className="w-5 h-5 rounded border-2 border-blue-500" /><span className="text-sm text-slate-600">Include Section</span></label></div>}
            <div className="text-center mb-12">
              <h2 className="text-4xl font-bold mb-4 text-blue-700">Campaigns & Results</h2>
              <p className="text-slate-600 max-w-2xl mx-auto">Notable projects and achievements</p>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">{(portfolioData.projects || []).map((project, idx) => {
              ...
              return <div key={idx} className={`relative bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 ${editMode && !isSelected ? 'opacity-40' : ''}`}>
                {editMode && <label className="absolute top-2 right-2 cursor-pointer z-10"><input type="checkbox" checked={isSelected} onChange={() => onToggleItem('projects', idx)} className="w-5 h-5 rounded border-2 border-blue-500" /></label>}
                ...
                <h3 className="font-bold text-xl text-blue-700 mb-2">{project.name || project.title || 'Project'}</h3>
        ...
        {templateState.included_sections.includes('education') && visibleEducation.length > 0 && (
          <section className="py-16 relative bg-white">
            {editMode && <div className="mb-4"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={true} onChange={() => onToggleSection('education')} className="w-5 h-5 rounded border-2 border-blue-500" /><span className="text-sm text-slate-600">Include Section</span></label></div>}
            <div className="text-center mb-12">
              <h2 className="text-4xl font-bold mb-4 text-blue-700">Education & Qualifications</h2>
            </div>
            <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">{(portfolioData.education || []).map((edu, idx) => {
              ...
              return <div key={idx} className={`relative bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl p-6 shadow-md hover:shadow-lg transition-shadow ${editMode && !isSelected ? 'opacity-40' : ''}`}>
                {editMode && <label className="absolute top-2 right-2 cursor-pointer z-10"><input type="checkbox" checked={isSelected} onChange={() => onToggleItem('education', idx)} className="w-5 h-5 rounded border-2 border-blue-500" /></label>}
                <div className="font-bold text-xl text-blue-700 mb-2">{edu?.degree || edu?.qualification || 'Degree'}</div>
                <div className="text-blue-600 mb-3 font-semibold">{edu?.institution || edu?.school || 'Institution'}{edu?.year && ` • ${edu.year}`}</div>
        ...
        {templateState.included_sections.includes('referees') && visibleReferees.length > 0 && (
          <section className="py-16 relative bg-gradient-to-br from-slate-50 to-blue-50">
            {editMode && <div className="mb-4"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={true} onChange={() => onToggleSection('referees')} className="w-5 h-5 rounded border-2 border-blue-500" /><span className="text-sm text-slate-600">Include Section</span></label></div>}
            <div className="text-center mb-12">
              <h2 className="text-4xl font-bold mb-4 text-blue-700">Professional References</h2>
            </div>
            <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">{(portfolioData.referees || []).map((ref, idx) => {
              ...
              return <div key={idx} className={`relative bg-white rounded-xl p-6 shadow-md hover:shadow-lg transition-shadow ${editMode && !isSelected ? 'opacity-40' : ''}`}>
                {editMode && <label className="absolute top-2 right-2 cursor-pointer z-10"><input type="checkbox" checked={isSelected} onChange={() => onToggleItem('referees', idx)} className="w-5 h-5 rounded border-2 border-blue-500" /></label>}
                <div className="font-bold text-xl text-blue-700 mb-2">{ref?.name || 'Referee'}</div>
                <div className="text-blue-600 mb-3 font-semibold">{ref?.title && `${ref.title} • `}{ref?.company || ''}</div>
        ...
        {templateState.included_sections.includes('attachments') && visibleAttachments.length > 0 && (
          <section className="py-16 relative bg-white">
            {editMode && <div className="mb-4"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={true} onChange={() => onToggleSection('attachments')} className="w-5 h-5 rounded border-2 border-blue-500" /><span className="text-sm text-slate-600">Include Section</span></label></div>}
            <div className="text-center mb-12">
              <h2 className="text-4xl font-bold mb-4 text-blue-700">Documents & Resources</h2>
            </div>
            <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">{(portfolioData.attachments || []).map((att, idx) => {
              ...
              return <div key={idx} className={`relative bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl p-6 shadow-md hover:shadow-lg transition-all hover:-translate-y-1 ${editMode && !isSelected ? 'opacity-40' : ''}`}>
                {editMode && <label className="absolute top-2 right-2 cursor-pointer z-10"><input type="checkbox" checked={isSelected} onChange={() => onToggleItem('attachments', idx)} className="w-5 h-5 rounded border-2 border-blue-500" /></label>}
                {url && <a href={url} target="_blank" rel="noopener noreferrer" className="block text-center">
                  ...
                  <div className="font-semibold text-blue-700 mb-1">{att?.title || 'Attachment'}</div>
        ...
        {templateState.included_sections.includes('social') && (portfolioData.socialLinks || []).length > 0 && (
          <section className="py-20 relative bg-gradient-to-br from-blue-600 via-cyan-600 to-blue-800 text-white">
            ...
            <div className="text-center max-w-4xl mx-auto">
              ...
              <p className="text-xl text-blue-100 mb-8">Get in touch through your preferred platform</p>
